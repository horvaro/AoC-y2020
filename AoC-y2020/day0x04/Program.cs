...
namespace day0x04
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var line in INPUT.Split(Environment.NewLine))
            {
                if (!string.IsNullOrWhiteSpace(line))
                {
                    ...
                    rawPassports.Add(tempPassport);
                    tempPassport = "";
                }
            }

            // Load last Passport after foreach
            if (tempPassport.Length > 0) rawPassports.Add(tempPassport);
        ...
        #region input
        public static string INPUT = @"byr:1937
